...
$post_status    		    = 'pending';	    //立即发布 pending 审核  draft 草稿箱
...
$secretWord     	    	= 'admin';	  //接口密码,如果不需要密码,则设为$secretWord=false ;	
...
if($hm_action== "list"){
  ...
  if (isset($secretWord)&&($secretWord!=false)) {
    if (!isset($_GET['secret']) || $_GET['secret'] != $secretWord) {
      wp_die('接口密码错误,请修改配置文件或者修改发布参数,保持两者统一。');
    }
  }

  extract($post);
  //判断标题是否为空
  if ($post_title=='[标题]'||$post_title=='') {wp_die('标题为空');}
  //检查标题是否重复
  if($checkTitle){
	  $post_title = trim(hm_strip_slashes($post_title));
    $sql = "SELECT `ID` FROM $wpdb->posts WHERE `post_title` = '$post_title'";
    $t_row = $wpdb->query($sql);
    if($t_row) {wp_die('标题重复,发布失败');};
  }
  //判断标题是否为空
  if ($post_content=='[内容]'||$post_content=='') {wp_die('内容为空');}
  //检查自定义文章类型
